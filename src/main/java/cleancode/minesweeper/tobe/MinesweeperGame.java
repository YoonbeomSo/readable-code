...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToPlantFlag(userActionInput)) {
            BOARD2[selectedRowIndex][selectedColIndex] = Cell.ofFlag();
            checkIfGameIsOver();
        ...
        if (doesUserChooseToOpenCell(userActionInput)) {
            if (isLandMindCell(selectedRowIndex, selectedColIndex)) {
                BOARD2[selectedRowIndex][selectedColIndex] = Cell.ofLandMine();
    ...
    private static void open(int row, int col) {
        ...
        if (BOARD2[row][col].doesNotClosed()) {
            return;
        }
        if (isLandMindCell(row, col)) {
            return;
        ...
        if (NEARBY_LAND_MINE_COUNTS[row][col] != 0) {
            BOARD2[row][col] = Cell.ofNearbyLandMineCount(NEARBY_LAND_MINE_COUNTS[row][col]);
            return;
        } else {
            BOARD2[row][col] = Cell.ofOpend();
        }
        open(row - 1, col - 1);
        open(row - 1, col);
        open(row - 1, col + 1);
